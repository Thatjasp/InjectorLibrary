...
class Injector {
public:
    Injector(std::shared_ptr<IBinaryFile<T>> binaryFile, std::shared_ptr<IBinaryFile<T>> payloadFile, CodeCave codeCave)
...
private:
    std::vector<char> getByteData(const std::shared_ptr<IBinaryFile<T>>& p_binary)
    {
    ...
    std::shared_ptr<IBinaryFile<T>> mp_payloadFile;
    std::shared_ptr<IBinaryFile<T>> mp_binaryFile;
